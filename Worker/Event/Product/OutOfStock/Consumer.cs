...
internal sealed class Consumer(IConsumer<string, Message> consumer, IOptions<ApisConfig> apisConfig) : IHostedService
{
    ...
    {
        consumer.Subscribe(Constants.ProductOutOfStockTopic);
        ...
        while (!_cancellationTokenSource.Token.IsCancellationRequested)
        {
            ...
            if (consumeResult.Message is not null)
            {
                ...
                var getOrderRequest = new RestRequest($"/orders/{consumeResult.Message.Value.SourceOrderID}", Method.Get);
...
{
    public static void AddProductOutOfStockEvent(this IServiceCollection services, IConfiguration configuration)
    {
